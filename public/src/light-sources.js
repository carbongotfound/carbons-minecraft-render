...
export function installLightSources(g){
 ...
 let last=0,stamp='';g.lighting={loaded:0,invalidate(){last=-Infinity;stamp='';},tick(t){
  if(t-last>700){last=t;const sources=[],p=g.playing?g.player:g.spawn,keys=new Set([...g.world.versions.keys(),...(g.world.v4Special||[])]);for(const chunk of view.chunks.values())for(const s of chunk.userData.emitters||[])keys.add(`${s.x},${s.y},${s.z}`);
   for(const key of keys){const[x,y,z]=key.split(',').map(Number);if(Math.abs(x-p.x)>64||Math.abs(z-p.z)>64||Math.abs(y-p.y)>64)continue;const distance=Math.hypot(x-p.x,y-p.y,z-p.z);if(distance>64)continue;const b=g.world.get(x,y,z),def=EMITTERS.get(b);if(!def)continue;sources.push({x,y,z,b,color:def[0],radius:def[1],distance});}
   ...
   if(next!==stamp){stamp=next;const before=new Set(previous.map(s=>`${s.x},${s.y},${s.z},${s.b}`)),after=new Set(sources.map(s=>`${s.x},${s.y},${s.z},${s.b}`)),changed=[...sources.filter(s=>!before.has(`${s.x},${s.y},${s.z},${s.b}`)),...previous.filter(s=>!after.has(`${s.x},${s.y},${s.z},${s.b}`))];for(const key of view.chunks.keys()){const[cx,cz]=key.split(',').map(Number),o=g.dimension==='overworld'?512:256;if(changed.some(s=>Math.abs(s.x-(cx*16-o+8))<32&&Math.abs(s.z-(cz*16-o+8))<32))g.world.dirty.add(key);}}
